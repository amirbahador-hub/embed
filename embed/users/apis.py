...
from embed.users.selectors import get_profile
...
from embed.users.validators import (
    integer_validator,
    alphabet_validator,
...
class ProfileApi(ApiAuthMixin, APIView):
    class OutPutProfileSerializer(serializers.ModelSerializer):
        ...
        def to_representation(self, instance):
            ...
            username = representation["user"]
            profile = cache.get(f"profile_{username}", {})

            if profile:
                representation["posts_count"] = profile.get("posts_count")
                representation["subscriptions_count"] = profile.get(
                    "subscriptions_count"
                )
                representation["subscribers_count"] = profile.get("subscribers_count")
